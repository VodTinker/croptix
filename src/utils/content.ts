...
// Cached DOM elements for iframe observer
let cachedVideo: HTMLVideoElement | null = null
let cachedControls: HTMLElement | null = null
let cachedFullscreen: HTMLElement | null = null
...
function createNewControl(btn: { id: string; label: string; svg: string }) {
...
// Create a new player control (will appear on the left of the fullscreen button)
function addNewControl(controlsContainer: HTMLElement | null, video: HTMLVideoElement | null, btn: { id: string; label: string; svg: string }) {
...
// Optimized version that uses cached fullscreen button
function addNewControlCached(
    controlsContainer: HTMLElement | null,
    video: HTMLVideoElement | null,
    fullscreenBtn: HTMLElement | null,
    btn: { id: string; label: string; svg: string }
) {
    if (!controlsContainer || !video || !fullscreenBtn) return
    if (document.getElementById(btn.id)) return

    const newControl = createNewControlCached(fullscreenBtn, btn)
    if (!newControl) return
...
// Uses cached fullscreen button instead of querying
function createNewControlCached(fullscreenBtn: HTMLElement, btn: { id: string; label: string; svg: string }) {
    const pipBtn = fullscreenBtn.cloneNode(true) as HTMLElement
...
function startObserverIframe() {
    ...
    const handleMutation = throttle(() => {
        // Skip if controls already initialized
        if (controlsInitialized) return

        // Use cached elements or query once
        if (!cachedVideo) {
            cachedVideo = document.getElementById('player0') as HTMLVideoElement | null
        }
        if (!cachedVideo) return

        if (!cachedControls) {
            cachedControls = document.getElementById('vilosControlsContainer') as HTMLElement | null
        }
        if (!cachedControls) return

        if (!cachedFullscreen) {
            cachedFullscreen = document.querySelector('[data-testid="vilos-fullscreen_button"]') as HTMLElement | null
        }

        pipControl(cachedControls, cachedVideo, cachedFullscreen)
        theaterControl(cachedControls, cachedVideo, cachedFullscreen)
...
function pipControl(controlsContainer: HTMLElement | null, video: HTMLVideoElement | null, fullscreenBtn: HTMLElement | null) {
    if (navigator.userAgent.toLowerCase().includes('firefox')) return

    const el = addNewControlCached(controlsContainer, video, fullscreenBtn, {
        id: 'vilos-pip_button',
...
function theaterControl(controlsContainer: HTMLElement | null, video: HTMLVideoElement | null, fullscreenBtn: HTMLElement | null) {
    const el = addNewControlCached(controlsContainer, video, fullscreenBtn, {
